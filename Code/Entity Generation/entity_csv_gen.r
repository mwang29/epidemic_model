...
#Belongs to
busData <- read.csv("Belongs_to.csv", header = T, stringsAsFactors = F)
s <- strsplit(busData$Regions, split = ",")
belongDf <- data.frame(Route = rep(busData$Route.Code, sapply(s, length)), Region = unlist(s))
write.csv(belongDf, file = "belongsTo.csv", row.names = F)
